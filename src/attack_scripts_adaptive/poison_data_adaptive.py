...
if not os.path.exists(path):
    print(f"[diag] {name}: missing file -> {path}")
    raise SystemExit(0)
total = 0
empty_pred = 0
multi_line = 0
with open(path, "r", encoding="utf-8") as f:
    for line in f:
        ...
        if "\n" in p:
            multi_line += 1
empty_ratio = (empty_pred * 100 / total) if total else 0.0
print(f"[diag] {name}: total={total}, empty_pred={empty_pred} ({empty_ratio:.2f}%), multiline={multi_line}")
PY
}
...
diagnose_stage() {
  ...
  local rule_postfix
  rule_postfix="$(echo "$(rule_file "$d")" | tr '/.' '__')"
  local clean_pred rand_pred ours_pred
  clean_pred="${PRED_ROOT}/${d}-clean-paper/${MODEL_NAME}/test/${rule_postfix}/predictions.jsonl"
  rand_pred="${PRED_ROOT}/${d}-rand-paper/${MODEL_NAME}/test/${rule_postfix}/predictions.jsonl"
  ours_pred="${PRED_ROOT}/${d}-ours-paper/${MODEL_NAME}/test/${rule_postfix}/predictions.jsonl"
  echo "[$d] diagnose prediction quality"
  diagnose_file "${d}-clean" "$clean_pred"
  diagnose_file "${d}-rand" "$rand_pred"
  diagnose_file "${d}-ours" "$ours_pred"
}
