...
def parse_args():
    ...
    # 输出文件名改为 only_nuke
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_only_nuke.jsonl")
...
# ==========================================
# 🚀 主程序 (Solo Module: Only Regex Nuke)
# ==========================================
def main():
    args = parse_args()
    ...
    print(f"🧪 开始 Solo 实验: 只保留正则核弹 (Only Regex Nuke)...")
    print(f"⚠️  泛洪已关闭，克隆已关闭。仅测试清洗能力。")

    with open(args.output_file, 'w') as f_out:
        success_count = 0

        # 为了快速验证，建议这里可以先跑前 20 条，如需全量请删掉 [:20]
        for idx, item in enumerate(tqdm(data[:200])):
            ...
            for i in range(target_idx + 1):
                ...
                # 注入毒药（如果 target_subjects 为空，这里就没东西注入）
                for p in poison_pack: new_graph.insert(0, p)
                # cloned_pack 是空的，不会插入
    ...
    print(f"✅ Solo 实验 (Regex Nuke) 完成！输出文件: {args.output_file}")
