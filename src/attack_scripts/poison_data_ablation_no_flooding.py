...
def parse_args():
    ...
    # 输出文件名改为 no_flooding
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_ablation_no_flooding.jsonl")
...
# ==========================================
# 🚀 主程序 (消融实验 E: w/o Degree Flooding)
# ==========================================
def main():
    args = parse_args()
    ...
    # 修改提示信息
    print(f"🧪 开始消融实验: 去除暴力泛洪 (w/o Degree Flooding)...")
    print(f"⚠️  警告: 此模式下毒药仅挂载在语义相关的节点，若找不到节点将不再强制挂在大V上！")
    ...
    with open(args.output_file, 'w') as f_out:
        ...
        for idx, item in enumerate(tqdm(data)):
            ...
            for i in range(target_idx + 1):
                ...
                # ⚠️ 关键点：如果 target_subjects 为空，这里 poison_pack 也会为空
                # 如果为空，实际上就没有注入毒药（除了可能的克隆边）
                ...
                item['strategy_analysis'] = {
                    "method": "Ablation_No_Flooding",  # 标记实验类型
    ...
    print(f"✅ 消融实验 (No Flooding) 完成！输出文件: {args.output_file}")
