...
def parse_args():
    ...
    # 输出文件名改为 only_flooding
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_only_flooding.jsonl")
...
# ==========================================
# 🚀 主程序 (Solo Module: Only Degree Flooding)
# ==========================================
def main():
    args = parse_args()
    ...
    print(f"🧪 开始 Solo 实验: 只保留暴力泛洪 (Only Degree Flooding)...")
    print(f"⚠️  核弹已关闭，克隆已关闭。仅测试泛洪召回能力。")
    ...
    with open(args.output_file, 'w') as f_out:
        ...
        for idx, item in enumerate(tqdm(data)):
            ...
            for i in range(target_idx + 1):
                ...
                # ==========================================================
                # 🔥 Solo 实验核心区：只保留 Degree Flooding
                # ==========================================================
                ...
                # ❌❌ [已移除] 拓扑克隆 (Topology Cloning) ❌❌
                cloned_pack = []  # 强制为空

                new_graph = []
                # ❌❌ [已移除] 正则核弹 (Regex Nuke) ❌❌
                # 我们不再进行专门的时间清洗，仅保留基础的真值清洗
                # is_time_q 判断逻辑被移除

                for t in original_graph:
                    head, rel, tail = t[0], t[1], t[2]

                    # 基础清洗：删除直接包含真值的边 (这是必要的，否则攻击太容易失效)
    ...
    print(f"✅ Solo 实验 (Degree Flooding) 完成！输出文件: {args.output_file}")
