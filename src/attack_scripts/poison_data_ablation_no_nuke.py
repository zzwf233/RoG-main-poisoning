...
def parse_args():
    ...
    # 注意：这里默认输出文件名改了，避免覆盖之前的成果
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_ablation_no_nuke.jsonl")
...
def main():
    ...
    # 修改提示信息，确认我们在跑消融
    print(f"🧪 开始消融实验: 去除正则核弹 (w/o Regex Nuke)...")
    print(f"⚠️  警告: 此模式下年份真值将被保留，预计 ASR 会下降！")
        ...
        for idx, item in enumerate(tqdm(data)):
            ...
            for i in range(target_idx + 1):
                ...
                for t in original_graph:
                    ...
                    # 3. ❌❌ [已移除] 正则核弹 (Regex Nuke) ❌❌
                    # 消融实验说明：我们故意注释掉了下面的代码，让 2014, 2012 等年份保留在图中
                    # -------------------------------------------------------------
                    # if is_time_q:
                ...
                item['strategy_analysis'] = {
                    "method": "Ablation_No_Nuke",  # 标记这是消融实验
    ...
    print(f"✅ 消融实验完成！输出文件: {args.output_file}")
    print(f"👉 下一步：请对 {args.output_file} 运行推理和评估脚本，查看 ASR 下降了多少。")
