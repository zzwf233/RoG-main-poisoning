...
def parse_args():
    ...
    # 输出文件名改为 only_reverse
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_only_reverse.jsonl")
...
# ==========================================
# 🚀 主程序 (Solo Module: Only Reverse Lookup)
# ==========================================
def main():
    args = parse_args()
    ...
    print(f"🧪 开始 Solo 实验: 只保留真值反向查找 (Only Reverse Lookup)...")
    print(f"⚠️  核弹❌，克隆❌，泛洪❌。只测试精准定位+基础注入的效果。")
        ...
        for idx, item in enumerate(tqdm(data[:200])):
            ...
            for i in range(target_idx + 1):
                ...
                for t in original_graph:
                    ...
                    # 基础清洗：删除直接包含真值的边
                    # 注意：这一步必须保留，否则图里有真值，模型100%选真值，ASR直接归零。
                    # 我们测的是“在没有真值干扰下，你的毒药是否有吸引力”。
    ...
    print(f"✅ Solo 实验 (Reverse Lookup) 完成！输出文件: {args.output_file}")
