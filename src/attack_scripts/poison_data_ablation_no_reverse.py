...
def parse_args():
    ...
    # 输出文件名改为 no_reverse
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_no_reverse.jsonl")
...
def main():
    ...
    print(f"🧪 开始消融实验: 去除真值反向查找 (w/o Reverse Lookup)...")
    print(f"⚠️  警告: 只能通过问题文本匹配实体，无法利用 Ground Truth 寻找挂载点！")

    with open(args.output_file, 'w') as f_out:
        success_count = 0

        # 这里使用全量，或者改为 data[:200]
        for idx, item in enumerate(tqdm(data[:200])):
            ...
            for i in range(target_idx + 1):
                ...
                # 1. 正常逻辑 (保留)
                # 只有这一步是“诚实”的攻击，完全依赖 LLM 对问题的理解和名字匹配
                ...
                item['strategy_analysis'] = {
                    "method": "Ablation_No_Reverse_Lookup",
    ...
    print(f"✅ 消融实验 (No Reverse Lookup) 完成！输出文件: {args.output_file}")
