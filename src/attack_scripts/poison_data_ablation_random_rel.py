...
def parse_args():
    ...
    # 输出文件名改为 random_rel
    parser.add_argument("--output_file", type=str, default="datasets/poisoned_cwq_ablation_random_rel.jsonl")
...
def main():
    ...
    # 修改提示信息
    print(f"🧪 开始消融实验: 去除关键关系泛洪 (w/o Pivot Relation)...")
    print(f"⚠️  警告: 此模式下使用随机/通用关系进行挂载，预计 ASR 会大幅下降！")
    ...
    with open(args.output_file, 'w') as f_out:
        ...
        for idx, item in enumerate(tqdm(data)):
            ...
            for i in range(target_idx + 1):
                ...
                # ✅✅ [替换] 随机或通用关系 ✅✅
                # 策略：强制使用一个“正确但无用”的描述性关系，
                # 或者完全随机选一个关系（如果你想测纯随机，可以用 random.choice(all_rels)）
                # 这里为了模拟“泛洪了但没打准”，我们使用一个虽然存在但语义宽泛的关系
                # 这样 RoG 在搜索特定路径（如 music.artist）时，就不会优先看这条边
                poison_rel = "common.topic.description"

                # 如果你想更彻底地测试“乱打”，可以用下面这一行代替上面那行：
                # if all_rels: poison_rel = random.choice(all_rels)
                ...
                for t in original_graph:
                    ...
                    # 注意：虽然我们在上面没有智能选择 risk_rels，
                    # 但为了控制变量，这里依然假设没有“精准打击”的正则清洗，只保留对毒药的泛洪
                    # 所以这里的 is_risk_edge 判断可以保留，因为 risk_rels 在上面没被赋值（或为空），不会误删
                ...
                item['strategy_analysis'] = {
                    "method": "Ablation_Random_Rel",  # 标记实验类型
    ...
    print(f"✅ 消融实验 (Random Relation) 完成！输出文件: {args.output_file}")
